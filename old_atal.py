    ...
    for batch, (image, target) in enumerate(metric_logger.log_every(data_loader, args.epoch_print_freq, header)):
        ...
        for _ in range(iteration_range):
            ...
            if class_pred_cache[0] == class_tar_cache[0]:
                epoch_corr_count = epoch_corr_count + 1
                corr_count = corr_count + 1

            it_count = it_count + 1
            epoch_it_count = epoch_it_count + 1

            main_acc = float(corr_count / it_count) * 100
            epoch_acc = (epoch_corr_count / epoch_it_count) * 100
            ...
            metric_logger.update(epoch_acc=epoch_acc)
            metric_logger.update(corr=epoch_corr_count)
            metric_logger.meters['acc1'].update(acc1.item(), n=iteration_range)
            metric_logger.meters['acc3'].update(acc3.item(), n=iteration_range)

            wandb.log({"lr": optimizer.param_groups[0]["lr"], "wd": optimizer.param_groups[0]["weight_decay"], 
                       "loss": loss.item(), "acc": main_acc, f"acc_epoch_{epoch + 1}": epoch_acc,
                       "acc1": acc1.item(), "acc3": acc3.item(),
                       "corr": corr_count, f"corr_epoch_{epoch + 1}": epoch_corr_count, 
                       "it": it_count, f"it_epoch_{epoch + 1}": epoch_it_count})
